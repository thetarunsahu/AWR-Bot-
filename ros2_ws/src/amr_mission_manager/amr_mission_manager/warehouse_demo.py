...
def main(args: list[str] | None = None) -> int:
    ...
    try:
        ...
        # The status subscriber already exists before this publish, so volatile
        # mission events are not missed as they were with ad-hoc ros2 topic echo.
        ...
        while rclpy.ok() and node.result_code is None:
            if time.monotonic() >= deadline:
                node.get_logger().error(
                    f'Mission timed out after {options.timeout:.0f}s.'
                )
                return 11
